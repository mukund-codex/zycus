<?php
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			$category_name = trim($func->escape_string($func->strip_all($_POST['category_name'])));
			
			if(empty($category_name)){
				header("location:".$pageURL."?registerfail&msg=Please enter a category_name");
				exit();
			}else {
				//add to database
				$result = $func->addCategory($_POST,$_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $func->getUniqueCategoryById($id);
	}
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			...
			if(empty($id) || empty($category_name)){
				header("location:".$pageURL."?updatefail&msg=Please enter a category_name&edit&id=".$id);
				exit();
			}
			else {
				//update to database
				$result = $func->updateCategory($_POST,$_FILES);
							...
							<div class="row">
							  ...
							  <!--  -->
								...
								<div class="col-sm-4">
									...
									<input type="text" class="form-control" id ="category_name" name="category_name" value="<?php if(isset($_GET['edit'])){ echo $data['category_name']; }  ?>" >			
							  </div>
								<div class="col-sm-4">
