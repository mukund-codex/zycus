<?php 
	...
	class SaveImage{
		...
		function uploadImageFileArrayFromForm($sourceFileElement,$num, $imageSizes, $imgDir, $resizeTo = 0, $targetFileName = ''){
			$targetFileName = (empty($targetFileName)) ? time() : $targetFileName;

			$fileNameArr = array();
			$fileName = str_replace(' ', '-', strtolower(pathinfo($sourceFileElement['name'][$num], PATHINFO_FILENAME)) );
			$ext = strtolower(pathinfo($sourceFileElement['name'][$num], PATHINFO_EXTENSION));
			$targetFileName .= $num;

			$imageProcessObj = new ImageProcess();
			$imageProcessObj->load($sourceFileElement["tmp_name"][$num]);
		...
		function uploadCroppedImageFileFromForm($sourceFileElement, $largeWidth, $cropData, $imgDir, $targetFileName = ''){
			...
			// == SAVE ORIGINAL IMAGE ==
			$imageProcessObj->load($sourceFileElement["tmp_name"]);
			// $imageProcessObj->resizeToWidth($largeWidth);   // commented by saurabh on 30/01/2019 for uploading original image without alteration
