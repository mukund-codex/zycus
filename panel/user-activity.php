...
<?php
						...
						while($row = $func->fetch($results)){ 
                            $category = $func->fetch($func->query("select * from ".PREFIX."category_master where id='".$row['category']."' "));

                            $activity = $func->fetch($func->query("select * from ".PREFIX."activity_master where id = '".$row['activity']."' "));

							$user = $func->fetch($func->query("select * from ".PREFIX."user_master where id='".$row['userid']."' "));
							
							if($row['status'] == 'already done' || $row['status'] == 'requested' || $row['status'] == 'interested'){
								if($row['approved'] == 'Approved' || $row['approved'] == 'Granted' || $row['status'] == 'interested' || $row['approved'] == 'Rejected'){
...
?>  
							<tr>
                                ...
                                <td>
									<a href="approve-activity.php?id=<?php echo $row['id']; ?>&status=<?php echo $row['status'] ?>" name="edit" class="" title="Click to Approve request" <?php echo $style; ?> ><i class="fa fa-check" aria-hidden="true" style="font-size:20px;"></i>&nbsp;&nbsp;&nbsp;&nbsp;</a>
									<a href="disapprove-activity1.php?id=<?php echo $row['id']; ?>" name="edit" class=""  title="Click to Disapprove request"  <?php echo $style; ?> ><i class="fa fa-times" aria-hidden="true" style="font-size:20px;"></i></a>
								</td>
							</tr>
<?php
						}
